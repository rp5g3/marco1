...
//		}
			...
			try{
				...
				if (!usuario.isNewUser()){
					...
					if (request.getSession(true).getAttribute("user") == null)
						throw new Exception("N�o � poss�vel atualizar um usu�rio n�o logado!");
					s.update(endereco);
					s.update(pessoa);
					s.update(tipoPessoa);
	...
	protected BindException validate(HttpServletRequest request) throws Exception {
		...
		if (usuario == null || usuario.getLogin() == null) throw new Exception("Usu�rio e login n�o podem ser nulos!");
		m = new HashMap();
		...
		if (errors == null || errors.getErrorCount() < 1){
			...
			System.out.println("Existem erros de formul�rio detectados na classe base");
		}
	...
	protected boolean canCadastreUsuario(HttpServletRequest request, Usuario usuario, BindException errors) throws Exception{
		...
		if (usuario.isNewUser()){
			if (usuario.getPassword().equals(usuario.getPasswordConf())){
				System.out.println("Validando o usu�rio");
				if( !(usuario.getPassword().length() < 6 || usuario.getPassword().length() > 8)){
					if (usuario.isNewUser()){
						...
						if (existe){
							...
							System.out.println("Usuario n�o existe e pode ser cadastrado");
							return true;
						}				
					}
					else return true;
