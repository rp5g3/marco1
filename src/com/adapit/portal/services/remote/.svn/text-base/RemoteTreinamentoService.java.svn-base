...
public class RemoteTreinamentoService implements TreinamentoService {

	private TreinamentoService treinamentoService;

	private static RemoteTreinamentoService instance;

	private RemoteTreinamentoService() {
		try {
			XmlBeanFactory beanFactory = SwingContext.getInstance()
					.getBeanFactory();
			treinamentoService = (TreinamentoService) beanFactory
					.getBean("remoteTreinamentoServiceHttpInvokerProxy");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}

	public static RemoteTreinamentoService getInstance() {
		if (instance == null) {
			instance = new RemoteTreinamentoService();
		}
		return instance;
	}
