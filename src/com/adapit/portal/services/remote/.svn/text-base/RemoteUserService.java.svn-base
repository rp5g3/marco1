...
public class RemoteUserService implements UserService {
	...
	private static RemoteUserService instance;
	...
	private RemoteUserService() {
		try {
			...
			userService = (UserService) beanFactory
					.getBean("remoteUserServiceHttpInvokerProxy");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	public static RemoteUserService getInstance() {
		if (instance == null) {
			instance = new RemoteUserService();
		}
		return instance;
	}
	...
	@Override
	public Usuario getUsuarioByIdCascadingProperties(java.io.Serializable id,
			boolean accessList, boolean deactivationReasons,
			boolean dadosPessoais) throws Exception {
		try {
			return this.userService.getUsuarioByIdCascadingProperties(id,
					accessList, deactivationReasons, dadosPessoais);
		} catch (Exception ex) {
			ex.printStackTrace();
			throw new Exception(
	...
	@Override
	public List<LoteReportDTO> getLotesParticipacao(String userid)
			throws Exception {
		try {
