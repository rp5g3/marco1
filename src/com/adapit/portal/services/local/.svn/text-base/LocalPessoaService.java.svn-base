...
public class LocalPessoaService implements PessoaService{
	...
	private static LocalPessoaService instance;

	private LocalPessoaService() {
		try {
			...
			pessoaService = (PessoaService) beanFactory
					.getBean("pessoaServiceDAOHibernate");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}

	public static LocalPessoaService getInstance() {
		if (instance == null) {
			instance = new LocalPessoaService();
		}
		return instance;
	}
