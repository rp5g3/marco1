...
public class InstrutorDadosPessoaisPanel extends JPanel{
	...
	CadastrarInstrutorForm cadastrarLeiloeiroForm;

	private JCheckBox naoValidarCpfCheckBox = null;

	private JLabel sexoLabel = null;

	private JComboBox sexoComboBox = null;

	public InstrutorDadosPessoaisPanel(CadastrarInstrutorForm cadastrarLeiloeiroForm){	
		this.cadastrarLeiloeiroForm = cadastrarLeiloeiroForm;
		initialize();
	}
	...
	private void initialize(){
		...
		sexoLabel.setBounds(new Rectangle(289, 92, 69, 20));
		...
		cargoLabel.setBounds(new Rectangle(289, 69, 100, 20));
		...
		this.setSize(new Dimension(577, 340));
		...
		this.add(sexoLabel, null);
		this.add(getSexoComboBox(), null);
	...
	protected JComponent getNomeTextField(){
		...
		if(nomeTextField == null){
			...
			this.binder.addBindProperty(this.instrutor, this.nomeTextField, "nome");
	...
	public void bindInstrutor(){
		instrutor.setTipoPessoa(fisica);
		instrutor.setNome(nomeTextField.getText());
		instrutor.setEmail(emailTextField.getText());
		instrutor.setTelefone(telefoneTextField.getText());
		instrutor.setFax(faxTextField.getText());	
		cadastrarLeiloeiroForm.apresentacaoParticipante.reverseBind();
	}
	...
	public boolean validatePessoaBean(){
		...
		Map errors = validate.validate(this.instrutor, "instrutor");
...
//Nunca definir um novo objeto entidade!!!
		instrutor.setTipoPessoa(null);
		instrutor.setEndereco(null);
		instrutor.setId(0);
		instrutor.setNome(null);
		instrutor.setEmail(null);
		instrutor.setTelefone(null);
		instrutor.setFax(null);
		...
		fisica.setProfissao("Leiloeiro");
		fisica.setRg(null);
		fisica.setSobrenome(null);

		binder.reverseBind(this.instrutor);
...
//Nunca passar como argumento novos objetos!!!
		try {
			//org.apache.commons.beanutils.BeanUtils.copyProperties(this.leiloeiro, objPessoa);
			
			instrutor.setId(objPessoa.getId());
			instrutor.setNome(objPessoa.getNome());
			instrutor.setEmail(objPessoa.getEmail());
			instrutor.setTelefone(objPessoa.getTelefone());
			instrutor.setFax(objPessoa.getFax());
			instrutor.setApresentacao(((Instrutor)objPessoa).getApresentacao());
			cadastrarLeiloeiroForm.apresentacaoParticipante.bind(instrutor);
		}catch(Exception e){
			e.printStackTrace();
		}
		try {
			...
			fisica.setProfissao("Colaborador da Adapit");
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	protected JComponent getEmailTextField(){
		...
		if(emailTextField == null){
			...
			this.binder.addBindProperty(this.instrutor, this.emailTextField, "email");
	...
	protected JComponent getTelefoneTextField(){
		...
		if(telefoneTextField == null){
			...
			this.binder.addBindProperty(this.instrutor, this.telefoneTextField, "telefone");
...
/*	private JScrollPane getCargoScrollPane() {
	...
	private JComponent getFaxTextField() {
		if (faxTextField == null) {
			...
			this.binder.addBindProperty(this.instrutor,this.faxTextField, "fax");			
	...
	/**
	 * This method initializes naoValidarCpfCheckBo	
	 * 	
	...
	private JCheckBox getNaoValidarCpfCheckBox() {
		if (naoValidarCpfCheckBox == null) {
			...
			naoValidarCpfCheckBox.setBounds(new Rectangle(289, 116, 230, 21));
			naoValidarCpfCheckBox.setText("Não validar cpf neste cadastro");
		}
	 ...
	 * @return javax.swing.JComboBox	
	 */
	private JComponent getSexoComboBox() {
		if (sexoComboBox == null) {
			sexoComboBox = new JComboBox();
			sexoComboBox.setBounds(new Rectangle(366, 91, 150, 22));
			Sexo vet[] = Sexo.values();
			for (int i =0 ; i < vet.length;i++){
				sexoComboBox.addItem(vet[i]);
			}
			this.binder.addBindProperty(this.fisica,this.sexoComboBox, "sexo");
			this.hashComps.put("sexo", this.sexoComboBox);
			JWarningComponent warn = new JWarningComponent(this.sexoComboBox);
			warn.setBounds(new Rectangle(366, 91, 150, 22));
			return warn;
		}
		return sexoComboBox;
	}
		...
		new java.lang.Thread(
			new Runnable(){
				 public void run(){
					...
					gui.add(new CadastrarInstrutorForm(),java.awt.BorderLayout.CENTER);
					gui.setVisible(true);
				}
			}
		).run();
