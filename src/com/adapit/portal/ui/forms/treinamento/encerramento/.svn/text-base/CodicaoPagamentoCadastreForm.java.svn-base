...
public class CodicaoPagamentoCadastreForm extends JPanel {
	 ...
	 */
	public CodicaoPagamentoCadastreForm() {
		super();
		initialize();
	}
	...
	private void initialize() {
		...
		contasGeradasLabel = new JLabel();
		contasGeradasLabel.setBounds(new Rectangle(291, 190, 137, 22));
		contasGeradasLabel.setText("Qtd de Contas Geradas:");
		...
		//this.add(getInicioPagto(), null);
		this.add(getEditarContasPagarButton(), null);
		this.add(contasGeradasLabel, null);
		this.add(getQtdContasGeradasTextField(), null);
	...
	/*private ParticipanteContaPagar gerarConta(Session s, int mes, int ano, float value){
		ParticipanteContaPagar c = new ParticipanteContaPagar();
		c.setAgendaLote(condicao.getAgendaLote());
		c.setCliente(condicao.getAgendaLote().getEncerramento().getLanceVencedor().getParticipante());
		c.setCondicaoPagamento(condicao);
		Date dt = new Date();
		dt.setDate(Integer.parseInt((String)getDiaComboBox().getSelectedItem()));
		dt.setMonth(mes);
	
		dt.setYear(ano);
		c.setDataVencimento(dt);
		c.setValor(value);
		return c;
	}*/
	

/*	private ParticipanteContaPagar gerarConta(Session s, Calendar cal, float value){
		ParticipanteContaPagar c = new ParticipanteContaPagar();
		c.setAgendaLote(condicao.getAgendaLote());
		c.setCliente(participante);
		c.setCondicaoPagamento(condicao);

		int dia = (Integer) getDiaComboBox().getSelectedItem();
	...
	/*private List<ParticipanteContaPagar> gerarVencimentoContas(Session s){
		List<ParticipanteContaPagar>  list = new ArrayList<ParticipanteContaPagar> ();
		if (avistaRadioButton.isSelected()){
			//Date dt = new Date();
			//int mesAtual = dt.getMonth();
			int car = (Integer) getCarenciaMeses().getValue();
			if (car > 0){
				Calendar cal = Calendar.getInstance();
				cal.add(Calendar.MONTH, car);
				list.add(gerarConta(s, cal, valorTotal));
			}
			else{
				Calendar cal = Calendar.getInstance();
			...
			int numeroPrest = (Integer) getNumeroPrestacoes().getValue();
			int carencias = (Integer) getCarenciaMeses().getValue();	
			int entradas = (Integer) getNumeroEntradas().getValue();
			//int mesInicio=0;
			//for (int i=0; i < numeroPrest; i++){
				...
				if (carencias > 0){
					for (int i=0; i < numeroPrest; i++){
						...
						list.add(gerarConta(s, cal, valorTotal/numeroPrest));
					}
				}
				else{					
					if (entradas > 0){
	...
	private void gerarContasPagar(){		
		int dia = (Integer) getDiaComboBox().getSelectedItem();
		boolean avista = avistaRadioButton.isSelected();
		int numeroPrest = (Integer) getNumeroPrestacoes().getValue();
		int carencias = (Integer) getCarenciaMeses().getValue();	
		int entradas = (Integer) getNumeroEntradas().getValue();
		try {
			condicao.getAgendaTreinamento();
			RemoteContaService.getInstance().gerarContasPagar(avista, valorTotal,
					condicao, participante, dia, carencias, numeroPrest, entradas);
			AdapitVirtualFrame.getInstance().showOperationSucess("Cadastro de Condição de Pagamento", 
					"Contas a pagar geradas com socesso");
		} catch (Exception e) {
			e.printStackTrace();
		}
		/*Session s = null;
		try {
			...
			s.beginTransaction();
			List<ParticipanteContaPagar> list = gerarVencimentoContas(s);
			for(ParticipanteContaPagar p : list){
				s.save(p);
			}
			s.getTransaction().commit();
			LeilaoVirtualFrame.getInstance().showOperationSucess("Cadastro de Condição de Pagamento", "Contas a pagar geradas com socesso");
		} catch (Exception e) {
			e.printStackTrace();
		}finally{
	...
	private void updateTotalPagar(){
		try {
			...
			if (getIncluirComissãoCheckBox().isSelected()){
				...
				float comiss = ((valorTotal*percent)/100);
				
				getComissaoLeiloeiroTextField().setText(Moeda.getValorReal(comiss));
				valorTotal = valorTotal+comiss;
			...
			if (getIncluirJurosCheckBox().isSelected()){
				...
				if (prest >0){
					for(int i=0; i < prest; i++)
						valorTotal = valorTotal+((valorTotal*percent)/100);
				}
			}
			...
			getValorTotalPagarTextField().setText(Moeda.getValorReal(valorTotal));
	...
	public JButton getSalvarButton() {
		if (salvarButton == null) {
			...
			salvarButton.setBounds(new Rectangle(5, 230, 80, 26));
	...
	public JButton getExcluirButton() {
		if (excluirButton == null) {
			...
			excluirButton.setBounds(new Rectangle(90, 230, 80, 26));
	...
	public JButton getGerarContasButton() {
		if (gerarContasButton == null) {
			...
			gerarContasButton.setBounds(new Rectangle(175, 230, 137, 26));
	...
	private JPanel getDadosAgendaPanel() {
		if (dadosAgendaPanel == null) {
			...
			gerarVencimentoLabel.setText("Gerar com vencimento de cada mês para o dia:");
			gerarVencimentoLabel.setBounds(new Rectangle(6, 190, 236, 22));
	...
	private JComboBox getDiaComboBox() {
		if (diaComboBox == null) {
			...
			diaComboBox.setBounds(new Rectangle(244, 190, 40, 22));
	...
	private JButton editarContasPagarButton = null;
	private JLabel contasGeradasLabel = null;
	private JTextField qtdContasGeradasTextField = null;
	...
	/**
	 * This method initializes editarContasPagarButton	
	 * 	
	 * @return javax.swing.JButton	
	 */
	private JButton getEditarContasPagarButton() {
		if (editarContasPagarButton == null) {
			editarContasPagarButton = new JButton();
			editarContasPagarButton.setBounds(new Rectangle(318, 230, 146, 26));
			editarContasPagarButton.setText("Editar Contas a Pagar");
			editarContasPagarButton.addActionListener(new ActionListener(){

				@Override
				public void actionPerformed(ActionEvent evt) {
					AdapitVirtualFrame.getInstance().editarContaPagarLote(agenda.getTreinamento());
				}
				
			});
		}
		return editarContasPagarButton;
	}
...
/*	private JSpinner qtdSpinner;
	...
	AgendaTreinamento agenda=null;
	public void updateContasButton(AgendaTreinamento agenda, Participante p){
		participante = p;
		this.agenda = agenda;
		if (encerramento != null && encerramento.getId() > 0){
			try {
				int size = RemoteContaService.getInstance().countParticipanteContaPagarByAgenda(agenda.getId());
				getQtdContasGeradasTextField().setText(size+"");
				if (size > 0){
					getEditarContasPagarButton().setEnabled(true);
					getGerarContasButton().setEnabled(false);
				}else{
					getDiaComboBox().setEnabled(true);
					getGerarContasButton().setEnabled(true);
					getEditarContasPagarButton().setEnabled(false);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
...
/*	public void updateContasButton(AgendaLote agenda, Participante p){
		participante = p;
		this.agenda = agenda;
		if (encerramento != null && encerramento.getId() > 0){
			Session s = null;
			try {
				...
				int size = (Integer) s.createQuery("select count(conta) from ParticipanteContaPagar conta where conta.agendaLote.id="+agenda.getId()).uniqueResult();
				getQtdContasGeradasTextField().setText(size+"");
				if (size > 0){
					getEditarContasPagarButton().setEnabled(true);
					getGerarContasButton().setEnabled(false);
	...
	public void updateAll(){
		editRegister(condicao);
		updateContasButton(agenda,participante);		
	}

	/**
	 * This method initializes qtdContasGeradasTextField	
	 * 	
	 * @return javax.swing.JTextField	
	 */
	private JTextField getQtdContasGeradasTextField() {
		if (qtdContasGeradasTextField == null) {
			qtdContasGeradasTextField = new JTextField();
			qtdContasGeradasTextField.setBounds(new Rectangle(433, 190, 30, 22));
			qtdContasGeradasTextField.setEditable(false);
			qtdContasGeradasTextField.setEnabled(false);
		}
		return qtdContasGeradasTextField;
	}
