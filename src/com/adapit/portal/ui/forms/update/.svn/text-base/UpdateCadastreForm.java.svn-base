...
public class UpdateCadastreForm extends JPanel {
	...
	private JTabbedPane getContentTabbedPane(){
		if (contentTabbedPane == null){
			...
			contentTabbedPane.add(getContentPanel(),"Dados gerais da versão");
			contentTabbedPane.add(getDescricaoEditor(),"Descrição/propaganda da versão (publicável  na internet)");
	 ...
	 private JPanel getContentPanel(){
		if (contentPanel == null){
			...
			resumoLabel.setText("Resumo da versão:");
			...
			resumoPanel.setBorder(BorderFactory.createTitledBorder("Informações resumidas da versão (publicável na internet)"));
			...
			//unPanel.add(new JLabel(""))
			unpublishedPanel.setBorder(BorderFactory.createTitledBorder("Informações não publicáveis na internet"));
			unpublishedPanel.add(getUnpublishedUpdateEditor());
			contentPanel.add(unpublishedPanel,null);
			
			dtLeilaoLabel= new JLabel("Data da versão:");
	...
	protected JComponent getDescricaoTextArea(){
		if(descricaoTextArea == null){
			...
			descricaoEventListener = new HtmlEditorEventListener(descricaoTextArea,AdapitVirtualFrame.getInstance(),"Texto da versão");
	...
	protected JButton getNewInstanceButton(){
		if(newInstanceButton == null){
			newInstanceButton = new JButton("Nova Instância Associada");
	...
	public void atualizarUpdate(){
		try {
			...
			if (e.getMessage() != null && e.getMessage().equals("update.categoria")){
				...
				JOptionPane.showMessageDialog(UpdateCadastreForm.this, "É necessário selecionar o lote.",
	...
	private JTabbedPane getTabbedPane() {
		if (tabbedPane == null) {
			...
			tabbedPane.addTab("Dados da versão", getUpdateContentPanel());
			tabbedPane.addTab("Imagens da versão", getImageComponentsPanel());
					...
					if (update.getId() <= 0){
						if (tabbedPane.getSelectedIndex() == 1){
							JOptionPane.showMessageDialog(AdapitVirtualFrame.getInstance(), 
									"Antes de inserir a imagem você precisa cadastrar a versão",
			...
			removerImagemButton.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					try {
						...
						JOptionPane.showMessageDialog(UpdateCadastreForm.this, "Este valor não é um número!", "O campo só aceita números", JOptionPane.ERROR_MESSAGE);
			...
			imgNumTextField.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					try {
						...
						if (n == 0){
							JOptionPane.showMessageDialog(UpdateCadastreForm.this, "Você deve informar um valor maior que zero!", "O campo só aceita números maiores que zero", JOptionPane.ERROR_MESSAGE);
							imgNumTextField.setText(""+(imgIndex+1));
						}else if (n > update.getImagens().size()){
							JOptionPane.showMessageDialog(UpdateCadastreForm.this, "Você deve informar um valor menor que "+n+"!", "Ordem incorreta das imagens", JOptionPane.ERROR_MESSAGE);
							imgNumTextField.setText(""+(imgIndex+1));
						}else{
			...
			anexarImagensButton.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent arg0) {
					if (update.getId() != 0){
						...
						JOptionPane.showMessageDialog(UpdateCadastreForm.this, "Primeiro é preciso cadastrar o update!","Adicionar imagem",JOptionPane.WARNING_MESSAGE);
	...
	private JComponent getResumoTextArea() {
		if (resumoTextArea == null) {
			...
			resumoUpdateEventListener = new HtmlEditorEventListener(resumoTextArea,AdapitVirtualFrame.getInstance(),"Resumo da versão");
	...
	private JComponent getUnpublishedTextArea() {
		if (unpublishedTextArea == null) {
			...
			unpublishedUpdateEventListener = new HtmlEditorEventListener(
					...
					"Dados não publicáveis da versão");
