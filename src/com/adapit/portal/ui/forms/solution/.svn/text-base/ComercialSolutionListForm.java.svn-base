...
public class ComercialSolutionListForm  extends JPanel implements CategoriaSelectable{
	...
	public ComercialSolutionListForm() {
		initialize();
	}
	
	public ComercialSolutionListForm(boolean sel) {
		this.selecionar = sel;
		initialize();
	}
	...
	private void pesquisar(){				
		...
		countFirst = RemoteComercialSolutionService.getInstance().countCommercialSolutionsAccordingTo(desc, byDescKind, c, vi, vf);
		criarListar();		
	}
	
	@SuppressWarnings("unchecked")
	private void listar(int firstResult){
		List prodList = RemoteComercialSolutionService.getInstance().listCommercialSolutionsAccordingTo(desc, byDescKind, c, vi, vf, firstResult);
				...
				public void actionPerformed(ActionEvent evt) {
					try {
						...
						if (row > -1){
							ComercialSolution p = getSelectedSolução();
							if (p == null) return;
							int resp = JOptionPane.showConfirmDialog(ComercialSolutionListForm.this, "Apagar esta solução implicará em apagar todo histório dela!"+'\n'+"Você quer mesmo assim apagar este item?", "Remover solução", JOptionPane.YES_NO_OPTION);
