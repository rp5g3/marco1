...
public class ComercialSolutionListFormBkp extends JPanel implements CategoriaSelectable{
	...
	public ComercialSolutionListFormBkp() {
		initialize();
	}
	
	public ComercialSolutionListFormBkp(boolean sel) {
		this.selecionar = sel;
		initialize();
	}
				...
				baseTable.addMouseListener(new MouseAdapter(){
					 public void mouseReleased(MouseEvent e) {
					    if ((e.getModifiers() & InputEvent.BUTTON1_MASK)!=0){
					    	ComercialSolution prod = getSelectedSolução();						
							/*if (prod.getComercialSolutionItens() != null && prod.getComercialSolutionItens().size() > 0){
							...
							getHistoricoProdButton().setEnabled(true);
					    }
					  }
				});
			}
	...
	@SuppressWarnings("unchecked")
	public ComercialSolution getSelectedSolução(){
		ComercialSolution prod = null;
		Iterator<ComercialSolution> it = getBaseTable().getElements().iterator();
		int row = getBaseTable().getSelectedRow();
		if (row < 0) return null;
		int id = (Integer) getBaseTable().getValueAt(row, 0);
		System.out.println(id);
		while(it.hasNext()){
			ComercialSolution p = it.next();
			if (p.getId() == id) return p;
		}
		return prod;
	}
	@SuppressWarnings("unchecked")
	public ComercialSolution getSelectedSolução(int row){
		ComercialSolution prod = null;
		Iterator<ComercialSolution> it = getBaseTable().getElements().iterator();
	...
	public JPanel getButtonsPanel() {
		if (buttonsPanel == null) {
			...
			if (!selecionar){
				...
				buttonsPanel.add(getHistoricoProdButton());
			}
		}
		return buttonsPanel;
	}
	...
	protected JButton getNovoComercialSolutionButton() {
		if (novoComercialSolutionButton == null) {
			novoComercialSolutionButton = new JButton("Nova Solução");
	...
	protected JButton getEditarSoluçãoButton() {
		...
		if (editarSoluçãoButton == null) {
			editarSoluçãoButton = new JButton("Editar Solução");
			...
			editarSoluçãoButton.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent evt) {
					try {
						...
						if (p == null) return;
						AdapitVirtualFrame.getInstance().editComercialSolution(p);						
	...
	protected JButton getRemoverButton() {
		if (removerButton == null) {
			removerButton = new JButton("Apagar a Solução");
				...
				public void actionPerformed(ActionEvent evt) {
					try {
						...
						if (row > -1){
							ComercialSolution p = getSelectedSolução();
							if (p == null) return;
							int resp = JOptionPane.showConfirmDialog(ComercialSolutionListFormBkp.this, "Apagar esta solução implicará em apagar todo histório dela!"+'\n'+"Você quer mesmo assim apagar este item?", "Remover solução", JOptionPane.YES_NO_OPTION);
	...
	protected JButton getRemoverSolDoTreinamentoButton() {
		if (removerSolDoTreinamentoButton == null) {
			removerSolDoTreinamentoButton = new JButton("Remover Solução do Treinamento");
	...
	private class RemoverSolucaoTreinamento extends AbstractAction{
		public void doAction(java.awt.event.ActionEvent e) {
			int row = getBaseTable().getSelectedRow();
			if (row >= 0)try{
				ComercialSolution produto = getSelectedSolução();
					...
					for(ComercialSolutionItem ip: produto.getComercialSolutionItens()){
						if (ip.getTreinamento() != null){
							...
							if (desc != null){
								...
								int resp = JOptionPane.showConfirmDialog(ComercialSolutionListFormBkp.this, "Você quer retirar a solução" + '\n'+desc + '\n'+" do treinamento " + lote.getCodigo() + "?","Remover Solução do Treinamento",JOptionPane.YES_NO_OPTION);
								if (resp == JOptionPane.YES_OPTION){
									
									RemoteComercialSolutionService.getInstance().removeTrainingSolutionItemFromScheduledTrainig(ip);
									
									resp = JOptionPane.showConfirmDialog(ComercialSolutionListFormBkp.this, "Apagar em definitivo a solução " + desc + "?","Remover Solução",JOptionPane.YES_NO_OPTION);
									if (resp == JOptionPane.YES_OPTION){
										try {
											RemoteComercialSolutionService.getInstance().generateTrainingSolutionHistory(produto, lote, ip, true);
											JOptionPane.showMessageDialog(ComercialSolutionListFormBkp.this, "Operação realizada com sucesso","Remover Solução",JOptionPane.INFORMATION_MESSAGE);
										} catch (Exception e1) {
											e1.printStackTrace();
											JOptionPane.showMessageDialog(ComercialSolutionListFormBkp.this, "Problemas ao gerar o histórico da solução!","Problema ao remover solução",JOptionPane.ERROR_MESSAGE);
										}
									}else {
										try {
											...
											e1.printStackTrace();
											JOptionPane.showMessageDialog(ComercialSolutionListFormBkp.this, "Problemas ao gerar o histórico da solução!","Problema ao remover a solução",JOptionPane.ERROR_MESSAGE);
										}
									}									
								}	
							}
	...
	protected JButton getTreinamentoarSoluçãosButton() {
		...
		if (lotearSoluçãosButton == null) {
			lotearSoluçãosButton = new JButton("Adicionar Solução em um Treinamento");
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					if (rows != null && rows.length > 0){
						for(int i=0; i < rows.length; i++){
							ComercialSolution p = (ComercialSolution) getSelectedSolução(rows[i]);//le.get(rows[i]);
							if (p==null) continue;
						...
						Iterator<ComercialSolution> it = le.iterator();
						while(it.hasNext()){
							ComercialSolution p = it.next();
							/*if (p.getComercialSolutionItens() != null && p.getComercialSolutionItens().size()>0){
								for(ComercialSolutionItem ip : p.getComercialSolutionItens()){
	...
	protected JButton getEditarTreinamentoButton() {
		...
		if (editarTreinamentoButton == null) {
			editarTreinamentoButton = new JButton("Editar o Treinamento do Solução");
					...
					if (row >=0){
						...
						/*if (p.getComercialSolutionItens() != null && p.getComercialSolutionItens().size()>0){
							for(ComercialSolutionItem ip : p.getComercialSolutionItens()){
								...
								if (LeilaoVirtualFrame.getInstance().lastCadastroTreinamentoInternalFrame != null){
									LeilaoVirtualFrame.getInstance().lastCadastroTreinamentoInternalFrame.getTreinamentoCadastreForm().setSelectedTurma(leilao);
									LeilaoVirtualFrame.getInstance().editarTreinamento(l,TreinamentoCadastreForm.formatDataTreinamento(leilao));
								}else{
									LeilaoVirtualFrame.getInstance().novoTreinamento();
									LeilaoVirtualFrame.getInstance().lastCadastroTreinamentoInternalFrame.getTreinamentoCadastreForm().setSelectedTurma(leilao);
									LeilaoVirtualFrame.getInstance().editarTreinamento(l,TreinamentoCadastreForm.formatDataTreinamento(leilao));
								}
							}
	...
	protected JButton getHistoricoProdButton() {

		if (historicoProdButton == null) {
			historicoProdButton = new JButton(messages
					.getMessage("VerHistóricodoSolução"));
			historicoProdButton.setSize(new java.awt.Dimension(150, 20));
			historicoProdButton.setIcon(new ImageIcon(getClass().getResource("/imgs/package_green.png")));
			historicoProdButton.setEnabled(false);
			historicoProdButton.setLocation(new java.awt.Point(0, 130));
		}
		return historicoProdButton;
	}
	...
	public class BaseTable extends JXTable {
		...
		public void updateTable() {
			...
			getTreinamentoarSoluçãosButton().setEnabled(false);
			getEditarTreinamentoButton().setEnabled(false);
			getHistoricoProdButton().setEnabled(false);
