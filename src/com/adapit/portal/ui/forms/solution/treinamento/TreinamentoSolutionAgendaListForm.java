package com.adapit.portal.ui.forms.solution.treinamento;
...
public class TreinamentoSolutionAgendaListForm extends JPanel implements CategoriaSelectable{
	...
	public TreinamentoSolutionAgendaListForm() {
		initialize();
	}
	
	public TreinamentoSolutionAgendaListForm(boolean sel) {
		this.selecionar = sel;
		initialize();
	}
				...
				public void actionPerformed(ActionEvent evt) {
					try {
						...
						if (row > -1){
							ComercialSolution p = getSelectedSolução();
							if (p == null) return;
							int resp = JOptionPane.showConfirmDialog(TreinamentoSolutionAgendaListForm.this, "Apagar esta solução implicará em apagar todo histório dela!"+'\n'+"Você quer mesmo assim apagar este item?", "Remover solução", JOptionPane.YES_NO_OPTION);
					...
					for(ComercialSolutionItem ip: produto.getComercialSolutionItens()){
						if (ip.getTreinamento() != null){
							...
							if (desc != null){
								...
								int resp = JOptionPane.showConfirmDialog(TreinamentoSolutionAgendaListForm.this, "Você quer retirar a solução" + '\n'+desc + '\n'+" do treinamento " + lote.getCodigo() + "?","Remover Solução do Treinamento",JOptionPane.YES_NO_OPTION);
								if (resp == JOptionPane.YES_OPTION){
									
									RemoteComercialSolutionService.getInstance().removeTrainingSolutionItemFromScheduledTrainig(ip);
									
									resp = JOptionPane.showConfirmDialog(TreinamentoSolutionAgendaListForm.this, "Apagar em definitivo a solução " + desc + "?","Remover Solução",JOptionPane.YES_NO_OPTION);
									if (resp == JOptionPane.YES_OPTION){
										try {
											RemoteComercialSolutionService.getInstance().generateTrainingSolutionHistory(produto, lote, ip, true);
											JOptionPane.showMessageDialog(TreinamentoSolutionAgendaListForm.this, "Operação realizada com sucesso","Remover Solução",JOptionPane.INFORMATION_MESSAGE);
										} catch (Exception e1) {
											e1.printStackTrace();
											JOptionPane.showMessageDialog(TreinamentoSolutionAgendaListForm.this, "Problemas ao gerar o histórico da solução!","Problema ao remover solução",JOptionPane.ERROR_MESSAGE);
										}
									}else {
										try {
											...
											e1.printStackTrace();
											JOptionPane.showMessageDialog(TreinamentoSolutionAgendaListForm.this, "Problemas ao gerar o histórico da solução!","Problema ao remover a solução",JOptionPane.ERROR_MESSAGE);
										}
									}									
								}	
							}
	...
	public static void main(String args[]) {
		...
		new java.lang.Thread(new Runnable() {
			public void run() {
				...
				gui.add(new TreinamentoSolutionAgendaListForm(), java.awt.BorderLayout.CENTER);
				gui.setVisible(true);
			}
		}).run();
	}
			...
			if (elements != null && elements.size() > 0) {
				...
				while (it.hasNext()) {
					...
					if (obj instanceof TrainingSolution) {
						TrainingSolution produto = (TrainingSolution) obj;
											...
											if (ip.getTreinamento().getTurma() != null){
												try {
													if (ip.getTreinamento().getTurma().getDataTreinamento() != null){
														...
														values[i][6]=AdapitVirtualFrame.getInstance().format(dt);
													}
												} catch (RuntimeException e) {
													e.printStackTrace();
												}
