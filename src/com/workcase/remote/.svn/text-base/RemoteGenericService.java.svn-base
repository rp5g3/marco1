...
public class RemoteGenericService implements GenericDAO {

	private GenericDAO genericService;

	private static RemoteGenericService instance;

	private RemoteGenericService() {
		try {
			XmlBeanFactory beanFactory = SwingContext.getInstance().getBeanFactory();
			genericService = (GenericDAO) beanFactory.getBean("remoteGenericDAOHttpInvokerProxy");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}

	public static RemoteGenericService getInstance() {
		if (instance == null) {
			instance = new RemoteGenericService();
		}
		return instance;
	}
