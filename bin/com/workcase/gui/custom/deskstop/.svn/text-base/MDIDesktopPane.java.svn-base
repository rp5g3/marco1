 ...
 */
package com.workcase.gui.custom.deskstop;
...
import com.workcase.gui.utils.ImageLoader;
...
public class MDIDesktopPane extends JDesktopPane {
    ...
    private Image im = ImageLoader.getInstance().getImage("icons/logo.gif");
    ...
    public void paintComponent(Graphics g) {
        ...
        if (this.manager.isScrollShowing()) {
            g.drawImage(im, lastPosition.x, lastPosition.y, this);
            return;
        }
        ...
        double width;
        if (dScreen.getWidth() < dDesktop.getWidth())
            width = dScreen.getWidth();
        else
            width = dDesktop.getWidth();
        
        double height;
        if (dScreen.getHeight() < dDesktop.getHeight())
            height = dScreen.getHeight();
        else
            height = dDesktop.getHeight();        
                
        int largura = (int)(width - im.getWidth(null)) / 2;
        int altura = (int)(height - im.getHeight(null)) / 2;

        lastPosition.setLocation(largura, altura);
        
        g.drawImage(im, largura, altura, this);
    }    
...
class MDIDesktopManager extends javax.swing.DefaultDesktopManager {
    ...
    private Insets getScrollPaneInsets() {
        ...
        Border b = getScrollPane().getBorder();
        if (b == null)
            return new Insets(0,0,0,0);
    ...
    public boolean isScrollShowing() {
        ...
        return (ext.getWidth() != view.getWidth()) || (ext.getHeight() != view.getHeight());
    }    
}
