...
public class RepresentanteLegalCadastreForm extends JPanel{
	
	private JTextField nomeTextField;
	
	private SwingBinder binder = new SwingBinder();
	
	private RepresentanteLegal representante = new RepresentanteLegal();
	...
	public RepresentanteLegalCadastreForm(){
		initialize();
	}
	...
	protected JComponent getNomeTextField(){
		...
		if(nomeTextField == null){
			...
			this.binder.addBindProperty(this.representante, this.nomeTextField, "nome");
	...
	public void bindFuncionario(){
		representante.setTipoPessoa(fisica);
		//funcionario.setId(0);
		representante.setNome(nomeTextField.getText());
		representante.setEmail(emailTextField.getText());
		representante.setTelefone(telefoneTextField.getText());
	...
	public Pessoa validatePessoaForm() throws Exception{
		...
		binder.bind(fisica);
		representante.setTipoPessoa(fisica);
		fisica.setPessoa(representante);
	...
	private JCheckBox getNaoValidarCpfCheckBox() {
		if (naoValidarCpfCheckBox == null) {
			...
			naoValidarCpfCheckBox.setBounds(new Rectangle(290, 116, 230, 21));
			naoValidarCpfCheckBox.setText("Não validar o cpf neste cadastro");
		}
	...
	public boolean validatePessoaBean(){
		...
		Map errors = validate.validate(this.representante, entityName);
...
//Nunca definir um novo objeto entidade!!!
		representante.setTipoPessoa(null);
		representante.setEndereco(null);
		representante.setId(0);
		representante.setNome(null);
		representante.setEmail(null);
		representante.setTelefone(null);
		representante.setFax(null);
		...
		binder.reverseBind(this.representante);
...
//Nunca passar como argumento novos objetos!!!
		...
		try {
			//org.apache.commons.beanutils.BeanUtils.copyProperties(this.representante, objPessoa);
		
			this.representante.setEmail(objPessoa.getEmail());
			this.representante.setEndereco(objPessoa.getEndereco());
			this.representante.setFax(objPessoa.getFax());
			this.representante.setNome(objPessoa.getNome());
			this.representante.setTelefone(objPessoa.getTelefone());
			this.representante.setId(objPessoa.getId());
		}catch(Exception e){
			e.printStackTrace();			
		}	
			...
			//Session s =null;
			try {
				objFisica = RemotePessoaService.getInstance().getPessoaFisicaByIdPessoa(objPessoa.getId());
				/*s = LocalServicesUtility.getInstance().openSession();
				objFisica = (Fisica) s.createQuery("from Fisica f where f.pessoa.id =" + objPessoa.getId()).uniqueResult();*/
				fisica.setId(objFisica.getId());
	...
	protected JComponent getEmailTextField(){
		...
		if(emailTextField == null){
			...
			this.binder.addBindProperty(this.representante, this.emailTextField, "email");
	...
	protected JComponent getTelefoneTextField(){
		...
		if(telefoneTextField == null){
			...
			this.binder.addBindProperty(this.representante, this.telefoneTextField, "telefone");
		...
		new java.lang.Thread(
			new Runnable(){
				 public void run(){
					...
					gui.add(new RepresentanteLegalCadastreForm(/*new Funcionario()*/),java.awt.BorderLayout.CENTER);
					gui.setVisible(true);
				}
			}
		).run();
