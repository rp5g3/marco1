...
public class SelecionarNewsListForm extends JDialog{
	...
	/*public Vector listaNewss(){
		Vector v = new Vector();
		Session s = LocalServicesUtility.getInstance().openSession();
		try{
			String query = "select News.id as PRODUTO_ID, News.descricao as PRODUTO_DESC," +
					"     Categoria.id as Cat_id, Categoria.nome as NOME_CATEGORIA," +
					" ItemNews.id as ItemNews_id, ItemNews.qtd, ItemNews.valorTotal," +
					" Lote.id as LOTE_ID, Lote.codlote," +
					" Leilao.id as Leilao_id, Leilao.datapresencialprimeira, Leilao.datapresencialsegunda " +
					//" ItemNews.valorTotal" +
					" from News, Categoria, ItemNews, Lote, Leilao" +
					" where    News.categoria_id = Categoria.id" +
					"     and News.id = ItemNews.news_id" +
					"     and ItemNews.lote_id = Lote.id" +
			...
			filtersPanel
					.setBorder(javax.swing.BorderFactory
							.createTitledBorder(javax.swing.BorderFactory
									.createTitledBorder("Filtrar newss de acordo com as seguintes regras")));
		...
		if (selecionarButton == null) {
			...
			selecionarButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (row >=0){
						news = (News) getBaseTable().getElements().get(row);
						dispose();
					}
				}
			});
				...
				while (it.hasNext()) {
					...
					if (obj instanceof com.adapit.portal.entidades.News) {
						...
						try{
							...
							values[i][1] = news.getDescricao();
							values[i][2] = news.getTitulo();
