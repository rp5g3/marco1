...
public class LocalNewsService implements NewsService {
	...
	private static LocalNewsService instance;

	private LocalNewsService() {
		try {
			XmlBeanFactory beanFactory = new XmlBeanFactory(new ClassPathResource("localServices.xml"));
			newsService = (NewsService) beanFactory.getBean("newsServiceDAOHibernate");//new LoteServiceDAOHibernate();
	...
	public static LocalNewsService getInstance() {
		if (instance == null) {
			instance = new LocalNewsService();
		}
		return instance;
	}
