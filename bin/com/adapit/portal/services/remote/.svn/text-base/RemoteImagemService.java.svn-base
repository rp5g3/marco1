...
public class RemoteImagemService implements ImagemService{
	private ImagemService imagemService;

	private static RemoteImagemService instance;

	private RemoteImagemService() {
		try {
			XmlBeanFactory beanFactory = SwingContext.getInstance()
					.getBeanFactory();
			imagemService = (ImagemService) beanFactory
					.getBean("remoteImagemServiceHttpInvokerProxy");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}

	public static RemoteImagemService getInstance() {
		if (instance == null) {
			instance = new RemoteImagemService();
		}
		return instance;
	}
