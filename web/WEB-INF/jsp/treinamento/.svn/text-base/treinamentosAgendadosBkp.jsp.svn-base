								      			...
								      			<TR bgcolor="<%=(count%2>0)?"#BBBBBB":"#CCCCCC" %>" onMouseOver="this.bgColor='AAAAAA';" onMouseOut="this.bgColor='<%=(count%2>0)?"#BBBBBB":"#CCCCCC" %>';">
													<TD >
														...
														<a onclick="javascript:dijit.byId('agendaDialog<%=lote.getId()+random%>').show(); return false;"
															...
															<br><a onclick="javascript:dijit.byId('agendaDialog<%=lote.getId()+random%>').show(); return false;"
															 		title="Clique para listar detalhes da execu&ccedil;&atilde;o deste treinamento"
															 		class="aBlack" 
															 		style="cursor: hand; cursor: pointer;"><Font color="Blue">Agenda</Font></a>
														 </center>
													</TD>
													<TD >
													...
													<TD >
														...
														<center>
															...
															<a onclick="modalImageWin('showImagensComercialSolution.html?produto_id=<%=lote.getTreinamento().getId()%>','Imagens do Treinamento <%=lote.getTreinamento().getNome() %>'); return false;"
															 		title="Clique para listar a descri&ccedil;&atilde;o deste treinamento"
															 		class="aBlack" 
															 		style="cursor: hand; cursor: pointer;">
															...
															<a onclick="addTab('treinamentosTabbedPane<%=leilaoId %>','itens<%=lote.getCodigo() %><%=random%>','Treinamento <%=lote.getTreinamento().getCodigo() %>','showDetalhesTreinamento.html?lote_id=<%=lote.getId()%>'); return false;"
															 		title="Clique para listar a descri&ccedil;&atilde;o deste treinamento"
															 		class="aBlack" 
															 		style="cursor: hand; cursor: pointer;">
													...
													<TD align="center"  >
														...
														<!-- <INPUT dojoType="dijit.form.CheckBox" 
																onClick="adapitPut('participarTurmaTreinamento.html?usuario_id=<%=user.getId()%>&treinamento_id=<%=lote.getId()%>'); return false;" type="checkbox"
																name="participarTreinamentoCheckBox<%=lote.getId()%>" style="width: 20px; height: 20px" <%if(lotesMarcados != null && lotesMarcados.contains(lote)){ %>checked="checked"<%} %>/>
														 -->
															<INPUT type="checkbox"  
...
<%
	...
	<div id="agendaDialog<%=lote.getId()+random%>" 
		...
		bindArgs="{sync: true, preventCache: false}"
		href="/adapit/showAgendaTreinamento.html?treinamento_id=<%=lote.getId()%>"/>	
